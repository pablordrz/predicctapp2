      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
        <Link to="/" className="flex items-center gap-2 shrink-0 group">
          <span className="w-8 h-8 rounded-lg bg-gradient-to-br from-accent to-accent2 flex items-center justify-center shadow-glow group-hover:scale-105 transition-transform">
            <span className="text-ink font-display font-bold text-sm">V</span>
          </span>
          <span className="font-display font-semibold text-lg tracking-tight">Vaticina</span>
        </Link>

        <nav className="hidden sm:flex items-center gap-6 text-sm text-muted font-medium">
          <Link to="/" className="hover:text-fg transition-colors">Mercados</Link>
          {user && <Link to="/crear" className="hover:text-fg transition-colors">Crear apuesta</Link>}
          {user?.is_admin && <Link to="/admin" className="hover:text-fg transition-colors">Panel admin</Link>}
        </nav>
        ...
        <div className="flex items-center gap-3">
          {user ? (
            ...
            <>
              ...
              <Link to="/registro" className="text-sm px-4 py-1.5 rounded-lg bg-accent hover:bg-accent2 transition-colors font-medium">
                Crear cuenta
              </Link>
            </>
          )}
